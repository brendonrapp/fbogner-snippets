...
while [ true ]; do
	...
	ping -c 1 "$SERVER_IP" &>/dev/null	#ping the server
	
	if [ "$?" -eq "0" ]; then	#and check if it's available
		# server reachable
		echo "$SERVER_IP is alive"
		
		kill -0 "$PID" &>/dev/null	#check if dns proxy is still running
		DNSSD_RUNNING=$?
		...
		if [ "$DNSSD_RUNNING" -ne "0" ]; then	# if we dont have a running dns-sd instance start it
			...
			dns-sd -P "$SERVER_NAME" "$SERVICE_TYPE" local $SERVICE_PORT "$SERVER_NAME.local" "$SERVER_IP" &>/dev/null &
			PID=$!;
		else 
